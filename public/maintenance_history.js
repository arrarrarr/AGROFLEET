document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadRecords(filters = {}) {
    try {
      ...
      const response = await fetch(
        `/api/maintenance/history${query ? `?${query}` : ""}`,
        {
          method: "GET",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  function renderRecords(data) {
    ...
    data.forEach((record) => {
      ...
      card.innerHTML = `
                <h3>${cardTitle}</h3>
                <p><i class="fas fa-calendar-alt"></i> <span>Дата: ${
                  record.date
                }</span></p>
                ${
                  record.reminder_label
                    ? `<p><i class="fas fa-bell"></i> <span>Нагадування: ${record.reminder_label}</span></p>`
                    : ""
                }
                <p><i class="fas fa-comment"></i> <span>Опис: ${
                  record.description
                }</span></p>
                <p><i class="fas fa-user"></i> <span>Відповідальний: ${
                  record.responsible
                }</span></p>
                      ...
                      userRole === "admin" || userRole === "manager"
                        ...
                        <button class="edit-btn noto-sans-regular" data-id="${
                          record.id
                            ...
                            record.reminder_label || record.type
                          )}">
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const url = editId
        ? `/api/maintenance/history/${editId}`
      ...
      if (!response.ok) {
        ...
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Помилка при створенні/оновленні запису"
        );
      }
