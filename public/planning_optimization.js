const API_URL = "https://agrofleet-pdqw.onrender.com/api/optimization";
const MOTO_TIME_API = "https://agrofleet-pdqw.onrender.com/api/moto_time";
...
function validateTaskForm(task) {
  ...
  if (!task.name || task.name.length < 3)
    errors.push("Назва завдання має бути не коротше 3 символів");
  if (!["technical_inspection", "repair"].includes(task.task_type))
    errors.push("Виберіть коректний тип завдання");
  if (
    !Number.isInteger(task.priority) ||
    task.priority < 1 ||
    task.priority > 3
  )
    errors.push("Пріоритет має бути числом від 1 до 3");
...
async function fetchTasks() {
  ...
  try {
    ...
    const updatedTasks = await fetch(url, { credentials: "include" }).then(
      (res) => res.json()
    );
...
async function fetchMotoTime(preserveSelection = false) {
  try {
    ...
    const selectedEquipmentFilter = preserveSelection
      ? equipmentFilter.value
      : "";
    const selectedEquipmentSelect = preserveSelection
      ? equipmentSelect.value
      : "";

    const uniqueEquipment = [
      ...new Set(records.map((record) => record.equipment)),
    ];
    ...
    if (preserveSelection) {
      if (
        ...
        uniqueEquipment.includes(selectedEquipmentFilter)
      ) {
        equipmentFilter.value = selectedEquipmentFilter;
      }
      if (
        selectedEquipmentSelect &&
        uniqueEquipment.includes(selectedEquipmentSelect)
      ) {
...
function calculateTimeRemaining(dueDate) {
  ...
  return diffHours >= 0
    ? `${diffHours} год`
    : `Просрочено на ${Math.abs(diffHours)} год`;
}
...
function updateKanbanBoard(tasks, motoTimeRecords) {
  ...
  if (!tasks || tasks.length === 0) {
    Object.values(columns).forEach(
      (column) =>
        (column.innerHTML = '<p class="empty-placeholder">Немає завдань</p>')
    );
    return;
  }
  tasks.forEach((task) => {
    ...
    if (task.dependencies) {
      const dependencyNames = task.dependencies
        .split(",")
        .map(
          (id) => tasks.find((t) => t.id === parseInt(id.trim()))?.name || id
        )
        .join(", ");
    ...
    if (columns[task.status]) {
      ...
      console.warn(
        `Завдання ${task.id} має некоректний статус: ${task.status}`
      );
    }
...
function updateCalendarView(tasks, motoTimeRecords) {
  ...
  document.getElementById("calendar-title").textContent = `${new Date(
    currentYear,
    currentMonth
  ).toLocaleString("uk", { month: "long" })} ${currentYear} р.`;
  if (!tasks || tasks.length === 0) {
  ...
  if (calendarMode === "month") {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        if ((i === 0 && j < adjustedFirstDay) || date > daysInMonth) {
          ...
          const taskDate = `${currentYear}-${String(currentMonth + 1).padStart(
            2,
            "0"
          )}-${String(date).padStart(2, "0")}`;
          const tasksOnDate = tasks.filter(
            (task) => task.due_date === taskDate
          );
          const motoRecordsOnDate = motoTimeRecords.filter(
            (record) => record.date === taskDate
          );
    ...
    div.innerHTML = `<h4>Завдання та використання техніки на ${date.toLocaleDateString(
      "uk"
    )}</h4>`;
    if (tasksOnDate.length > 0 || motoRecordsOnDate.length > 0) {
      const taskList = document.createElement("div");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.addEventListener("message", async (event) => {
    if (
      event.data.type === "taskUpdated" ||
      event.data.type === "taskDeleted"
    ) {
      await fetchTasks();
    }
  });
  ...
  document
    ...
    .addEventListener("click", optimizeTasks);
  document
  ...
  document
    ...
    .addEventListener("submit", async (e) => {
      ...
      const selectedDependencies = Array.from(
        dependenciesSelect.selectedOptions
      )
