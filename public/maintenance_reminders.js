document.addEventListener('DOMContentLoaded', () => {
    ...
    const equipmentParts = {
        'Трактор 404D6ZU': ['Двигун', 'Гідравліка', 'Колеса', 'Трансмісія', 'Кабіна'],
        'Трактор Xingtai XT-900': ['Двигун', 'Гідравліка', 'Колеса', 'Трансмісія', 'Система охолодження'],
        'Комбайн CAT Lexion 470R': ['Жатка', 'Молотильний барабан', 'Система сепарації', 'Двигун', 'Гідравліка'],
        'Комбайн YTO 4LZ-8B1': ['Жатка', 'Молотильний барабан', 'Система сепарації', 'Двигун', 'Гідравліка'],
        'Дощувальна машина ДТЗ 300': ['Насос', 'Форсунки', 'Труби', 'Електродвигун', 'Контролер'],
        'Дощувальна машина ПГР-3.35': ['Насос', 'Форсунки', 'Труби', 'Електродвигун', 'Контролер'],
        'Дощувальна машина ПГР НН-225': ['Насос', 'Форсунки', 'Труби', 'Електродвигун', 'Контролер'],
        'Дощувальна машина ПГР НН 220': ['Насос', 'Форсунки', 'Труби', 'Електродвигун', 'Контролер'],
    ...
    const loadReminders = async (filters = {}) => {
        try {
            ...
            const response = await fetch(`/api/maintenance/reminders${query ? `?${query}` : ''}`, {
                method: 'GET',
                credentials: 'include',
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Помилка при завантаженні нагадувань');
            }
    ...
    const displayReminders = (data) => {
        ...
        data.forEach(reminder => {
            ...
            card.className = `reminder-card ${reminder.completed ? 'completed' : ''} noto-sans-regular`;
            card.innerHTML = `
                <h3>${reminder.text}</h3>
                <p><i class="fas fa-calendar-alt"></i> <span>Дата: ${formatDate(reminder.dateTime)}</span></p>
                <p><i class="fas fa-tractor"></i> <span>Техніка: ${reminder.equipment || 'Невідомо'}</span></p>
                <p><i class="fas fa-user"></i> <span>Оператор: ${reminder.operator || 'Невідомо'}</span></p>
                <p><i class="fas fa-tools"></i> <span>Тип: ${reminder.taskType === 'maintenance' ? 'Техогляд' : 'Ремонт'}</span></p>
                ${reminder.taskType === 'repair' && reminder.repairPart ? `
                    <p><i class="fas fa-wrench"></i> <span>Деталь: ${reminder.repairPart}</span></p>
                    <p><i class="fas fa-comment"></i> <span>Опис ремонту: ${reminder.repairDescription || 'Немає'}</span></p>
                ` : ''}
                <p><i class="fas fa-info-circle"></i> <span>Статус: <span class="status-label ${reminder.status}">${getStatusText(reminder.status)}</span></span></p>
                <div class="action-buttons">
                    ${userRole === 'admin' || userRole === 'manager' ? `
                        <button class="complete-btn noto-sans-regular" data-id="${reminder.id}">
                            <i class="fas fa-check"></i> ${reminder.completed ? 'Відновити' : 'Завершити'}
                        </button>
                        <button class="edit-btn noto-sans-regular" data-id="${reminder.id}" data-text="${encodeURIComponent(reminder.text)}">
                            <i class="fas fa-edit"></i> Редагувати
                        </button>
                        <button class="delete-btn noto-sans-regular" data-id="${reminder.id}">
    ...
    const editReminder = (id) => {
        ...
        form.taskDateTime.value = reminder.dateTime ? reminder.dateTime.slice(0, 16) : '';
        ...
        repairDetails.style.display = reminder.taskType === 'repair' ? 'block' : 'none';
        updateRepairParts();
    ...
    const updateReminderStatus = async (id) => {
        try {
            ...
            showToast(completed ? 'Нагадування завершено та перенесено до завершених задач.' : 'Нагадування відновлено та додано до задач оптимізації.');
            await loadReminders({ date: dateFilter.value, status: statusFilter.value });
            await fetchTasks();
        } catch (error) {
    ...
    const deleteReminder = async (id) => {
        ...
        try {
            ...
            showToast('Нагадування видалено разом із пов’язаною задачею для оптимізації.');
            await loadReminders({ date: dateFilter.value, status: statusFilter.value });
            await fetchTasks();
        } catch (error) {
    ...
    taskTypeSelect.addEventListener('change', () => {
        repairDetails.style.display = taskTypeSelect.value === 'repair' ? 'block' : 'none';
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const reminder = {
            ...
            repairPart: form.taskType.value === 'repair' ? form.repairPart.value : null,
            repairDescription: form.taskType.value === 'repair' ? form.repairDescription.value : null,
        };

        console.log('Дані для відправки:', reminder);

        if (!reminder.text || !reminder.dateTime || !reminder.equipment || !reminder.operator || !reminder.taskType) {
        ...
        try {
            const url = editId ? `/api/maintenance/reminders/${editId}` : '/api/maintenance/reminders';
            ...
            if (!response.ok) {
                ...
                const errorData = await response.json();
                throw new Error(errorData.error || 'Помилка при створенні/оновленні нагадування');
            }

            const updatedReminder = await response.json();
            showToast(editId ? 'Нагадування оновлено' : 'Нагадування додано та автоматично створено як задачу для оптимізації.');
            modal.style.display = 'none';
            modal.style.animation = '';
            form.reset();
            repairDetails.style.display = 'none';
    ...
    async function fetchTasks() {
        try {
            const response = await fetch('http://localhost:3000/api/optimization/tasks', { credentials: 'include' });
