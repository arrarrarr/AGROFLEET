document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkAuth = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                console.log('Не авторизований. Статус:', response.status, 'Повідомлення:', data.error);
                localStorage.setItem('authMessage', data.error || 'Будь ласка, увійдіть в систему');
    ...
    const loadUserProfile = async () => {
        try {
            ...
            userRole = user.role;
            currentUserId = user.id;
            console.log('Роль користувача:', userRole, 'ID користувача:', currentUserId);
    ...
    const fetchExpenses = async (filters = {}) => {
        ...
        try {
            ...
            const response = await fetch(`/api/expenses?${query}`, { credentials: 'include' });
            if (!response.ok) {
                let errorMessage = 'Помилка завантаження витрат';
                try {
                    const errorData = await response.json();
                    errorMessage = errorData.error || errorMessage;
                    console.log('Помилка від сервера:', errorMessage, 'Статус:', response.status);
                    if (response.status === 401) {
                        localStorage.setItem('authMessage', 'Сесія закінчилася. Будь ласка, увійдіть знову.');
                        window.location.href = '/index.html';
                        return;
                    }
                } catch (e) {
                    console.log('Не вдалося отримати повідомлення про помилку:', e.message);
                }
            ...
            if (expenses.length === 0) {
                console.log('Сервер повернув порожній масив. Перевірте базу даних або права доступу.');
            }
            renderGrid(expenses);
            ...
            grid.innerHTML = '<p>Не вдалося завантажити витрати. Спробуйте ще раз.</p>';
        } finally {
            loadingIndicator.style.display = 'none';
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            if (!editId) {
                if (!expense.equipmentName || !expense.expenseType || !expense.expenseAmount || !expense.expenseDate) {
                    console.log('Недостатньо даних для створення нової витрати:', expense);
            ...
            if (isNaN(expense.expenseAmount) || expense.expenseAmount < 0) {
                console.log('Некоректне значення expenseAmount:', expense.expenseAmount);
            ...
            try {
                ...
                if (editId) {
                    console.log('Надсилаємо PUT-запит для редагування запису з id:', editId);
                ...
                modal.style.display = 'none';
                form.reset();
                fetchExpenses({ expenseType: filterType.value, period: exportPeriod.value, day: dayPicker.value, week: weekPicker.value, month: monthPicker.value, halfyear: halfyearPicker.value, year: yearPicker.value, startDate: startDate.value, endDate: endDate.value });
    ...
    exportPeriod.addEventListener('change', () => {
        dayPicker.style.display = exportPeriod.value === 'day' ? 'inline-block' : 'none';
        weekPicker.style.display = exportPeriod.value === 'week' ? 'inline-block' : 'none';
        monthPicker.style.display = exportPeriod.value === 'month' ? 'inline-block' : 'none';
        halfyearPicker.style.display = exportPeriod.value === 'halfyear' ? 'inline-block' : 'none';
        yearPicker.style.display = exportPeriod.value === 'year' ? 'inline-block' : 'none';
        startDate.style.display = exportPeriod.value === 'custom' ? 'inline-block' : 'none';
        endDate.style.display = exportPeriod.value === 'custom' ? 'inline-block' : 'none';
        fetchExpenses({ expenseType: filterType.value, period: exportPeriod.value, day: dayPicker.value, week: weekPicker.value, month: monthPicker.value, halfyear: halfyearPicker.value, year: yearPicker.value, startDate: startDate.value, endDate: endDate.value });
    });
    ...
    [weekPicker, monthPicker, halfyearPicker, yearPicker, startDate, endDate].forEach(picker => {
    ...
    function renderGrid(data) {
        ...
        data.forEach((e, index) => {
            ...
            const iconClass = expenseIcons[e.expense_type.toLowerCase()] || 'fas fa-question';
            const fullName = e.fullName || 'Невідомий';
            let card = `
                <div class="expense-card">
                    <h3><i class="${iconClass}"></i> ${e.expense_type}</h3>
                    <p><i class="fas fa-tractor"></i> Техніка: ${e.equipment_name}</p>
                    <p><i class="fas fa-user"></i> Користувач: ${fullName}</p>
                    <p><i class="fas fa-money-bill-wave"></i> Сума: ${formatNumber(e.expense_amount.toFixed(2))} UAH</p>
                    <p><i class="fas fa-calendar-alt"></i> Дата: ${formatDate(e.expense_date)}</p>
                    <div class="actions">
            `;

            if (userRole === 'admin' || (userRole === 'manager' && e.user_id === currentUserId)) {
                card += `
    ...
    function updateTotalExpenses(filtered) {
        ...
        if (exportPeriod.value !== 'all') {
            if (exportPeriod.value === 'day' && dayPicker.value) {
                ...
                title += ` за ${halfyearPicker.options[halfyearPicker.selectedIndex].text}`;
            } else if (exportPeriod.value === 'year' && yearPicker.value) {
                title += ` за ${yearPicker.value}`;
            } else if (exportPeriod.value === 'custom' && startDate.value && endDate.value) {
                title += ` з ${formatDate(startDate.value)} по ${formatDate(endDate.value)}`;
            }
        }
        ...
        const interval = setInterval(() => {
            ...
            totalExpensesElement.textContent = `${formatNumber(current.toFixed(2))} UAH`;
        }, 20);
    }
    ...
    (async () => {
        ...
        fetchExpenses({ expenseType: filterType.value, period: exportPeriod.value });
    })();
});