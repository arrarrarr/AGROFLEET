export function initializeInventory() {
  ...
  function setupInventory() {
    ...
    const fetchOrderHistory = async () => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            errorData.error || "Помилка завантаження історії замовлень"
          );
        }
        const contentType = response.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
    ...
    const deleteOrder = async (id) => {
      if (
        !confirm(
          "Ви впевнені, що хочете видалити цей запис з історії замовлень?"
        )
      )
        return;
      try {
    ...
    const renderOrderHistory = (orders) => {
      ...
      orders.forEach((order) => {
        ...
        orderItem.innerHTML = `
                    ...
                    <p><i class="fas fa-cubes"></i> Кількість: ${
                      order.quantity
                    }</p>
                    <p><i class="fas fa-money-bill-wave"></i> Загальна сума: ${formatNumberWithSpaces(
                      order.total_cost
                    )} UAH</p>
                    <p><i class="fas fa-calendar-alt"></i> Дата: ${formatDate(
                      order.date
                    )}</p>
                    <p><i class="fas fa-user"></i> Відповідальний: ${
                      order.responsible
                    }</p>
                    <p><i class="fas fa-sticky-note"></i> Примітки: ${
                      order.notes || "Немає"
                    }</p>
                    ${
                      userRole === "admin" ||
    ...
    const renderGrid = (data) => {
      ...
      data.forEach((item) => {
        ...
        const status =
          item.quantity <= item.min_level ? "Низький рівень" : "Норма";
        ...
        card.className = `expense-card ${
          status === "Низький рівень" ? "low-stock" : ""
        }`;
        card.innerHTML = `
                    ...
                    <p><i class="fas fa-exclamation-triangle"></i> Мінімальний рівень: ${
                      item.min_level
                    }</p>
                    <p><i class="fas fa-money-bill-wave"></i> Сума: ${formatNumberWithSpaces(
                      totalItemCost
                    )} UAH</p>
                    <p><i class="fas fa-calendar-alt"></i> Дата: ${formatDate(
                      item.update_date
    ...
    const saveInventory = async (e) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            errorMessage =
              ...
              errorData.error ||
              `Помилка ${editId ? "редагування" : "додавання"} запаса`;
          } catch {
        ...
        if (updatedItem.quantity <= updatedItem.min_level) {
          alert(
            `Увага: Низький рівень запаса "${updatedItem.name}"! Кількість: ${updatedItem.quantity}, Мінімальний рівень: ${updatedItem.min_level}`
          );
        }
    ...
    const exportToExcel = () => {
      ...
      const fileName = `Історія_замовлень_${
        new Date().toISOString().split("T")[0]
      }`;
      const wsData = orders.map((order) => ({
        Назва: order.name,
        Кількість: order.quantity,
        "Вартість за одиницю (UAH)": formatNumberWithSpaces(
