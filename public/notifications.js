...
async function loadNotifications() {
  ...
  const notificationBadgeProfile = document.getElementById(
  ...
  try {
    ...
    if (userData.role === "unauthorized") {
      console.log(
        "Користувач не авторизований при завантаженні сповіщень, відкриваємо authModal"
      );
      showModal("authModal");
      notificationsList.innerHTML =
        "<p>Будь ласка, увійдіть, щоб переглянути сповіщення.</p>";
      updateMessageIcons(
        messageIconNav,
    ...
    if (
      ...
      notificationsList.innerHTML =
        "<p>Сповіщення доступні лише для користувачів, менеджерів та адміністраторів.</p>";
      updateMessageIcons(
        messageIconNav,
        notificationBadgeNav,
        messageIconProfile,
    ...
    const response = await fetch(
      "https://agrofleet-pdqw.onrender.com/api/notifications",
      {
        method: "GET",
        credentials: "include",
      }
    ...
    if (!response.ok) {
      ...
      notificationsList.innerHTML = `<p>Помилка завантаження сповіщень: ${
                    ...
                    ${notifications
                      .map((notification) => {
                        const createdAt = notification.created_at
                          ? new Date(notification.created_at + "Z")
                          : null;
                        const formattedDate =
                          createdAt && !isNaN(createdAt)
                            ...
                            : "Дата не вказана";
                        const orderNumber =
                          notification.relatedId || "невідомий";
                        const userName =
                          notification.user?.fullName || "Невідомий користувач";
                        const totalAmount =
                          notification.order &&
                          notification.order.totalAmount != null
                            ? notification.order.totalAmount.toLocaleString(
                                "uk-UA"
                              )
                            : "0";
                        ...
                        const address =
                          notification.user?.address || "невідома";
                        const city = notification.user?.city || "невідоме";
                        const rentalStart =
                          notification.order && notification.order.rentalStart
                            ? new Date(
                                notification.order.rentalStart
                              ).toLocaleDateString("uk-UA")
                            : "невідомо";
                        const rentalEnd =
                          notification.order && notification.order.rentalEnd
                            ? new Date(
                                notification.order.rentalEnd
                              ).toLocaleDateString("uk-UA")
                            : "невідомо";
                        ...
                        if (notification.order && notification.order.items) {
                          if (typeof notification.order.items === "string") {
                            try {
                              ...
                              console.error(
                                `Ошибка парсинга items для уведомления ${notification.id}:`,
                                err.message
                              );
                              items = [];
                            }
                        ...
                        if (userRole === "user") {
                          return `
                                <div class="notification-item">
                                    ...
                                    <div class="notification-actions">
                                        <button class="action-button cancel-button" data-order-id="${
                          ...
                          return `
                                <div class="notification-item">
                                    ...
                                    <div class="notification-actions">
                                        <button class="action-button confirm-button" data-order-id="${
                                          notification.relatedId || ""
                            ...
                            notification.relatedId || ""
                                        ...
                                        <button class="action-button decline-button" data-order-id="${
                                          notification.relatedId || ""
...
async function markNotificationsAsRead() {
  try {
    const response = await fetch(
      "https://agrofleet-pdqw.onrender.com/api/notifications/mark-read",
      {
        method: "PUT",
        credentials: "include",
      }
...
window.confirmOrder = async (notificationId, relatedId) => {
  ...
  try {
    ...
    const response = await fetch(
      "https://agrofleet-pdqw.onrender.com/api/notifications/confirm-to-contract",
      {
        method: "POST",
    ...
    console.log(
      "Ответ от /api/notifications/confirm-to-contract:",
      responseData
    );

    // Обновляем статус заказа
    const confirmOrderResponse = await fetch(
      `https://agrofleet-pdqw.onrender.com/api/orders/${relatedId}/confirm`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    showNotification(
      "Замовлення успішно підтверджено! Переходимо до створення договору."
    );
...
window.declineOrder = async (notificationId, relatedId) => {
  ...
  try {
    const response = await fetch(
      `https://agrofleet-pdqw.onrender.com/api/orders/${relatedId}/decline`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    console.log(
      `Ответ от /api/orders/${relatedId}/decline:`,
      response.status,
      response.statusText
    );
    if (!response.ok) {
...
window.cancelOrder = async (notificationId, relatedId) => {
  ...
  try {
    const response = await fetch(
      `https://agrofleet-pdqw.onrender.com/api/orders/${relatedId}/cancel`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    console.log(
      `Ответ от /api/orders/${relatedId}/cancel:`,
      response.status,
      response.statusText
    );
    if (!response.ok) {
...
async function deleteNotification(notificationId) {
  try {
    const response = await fetch(
      `https://agrofleet-pdqw.onrender.com/api/notifications/${notificationId}`,
      {
        method: "DELETE",
        credentials: "include",
      }
    ...
    console.log(
      `Ответ от /api/notifications/${notificationId}:`,
      response.status,
      response.statusText
    );
    if (!response.ok) {
