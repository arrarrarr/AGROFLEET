...
async function checkAuth() {
  try {
    ...
    console.log(
      "Відповідь від /api/auth/check-auth (checkAuth):",
      response.status,
      response.statusText
    );
    if (!response.ok) {
...
const fetchUnreadNotificationsCount = async () => {
  try {
    const response = await fetch(
      "https://agrofleet-pdqw.onrender.com/api/notifications/unread-count",
      {
        method: "GET",
        headers: {
    ...
    console.error(
      "Помилка при завантаженні кількості непрочитаних сповіщень:",
      err.message
    );
    const badge = document.getElementById("notificationBadgeProfile");
    const messageIcon = document.getElementById("messageIconProfile");
                  ...
                  .map(
                    ...
                    <div class="notification-item" data-id="${notification.id}">
                        <div class="notification-content">
                            ...
                            } для користувача ${notification.user.fullName}.</p>
                            <p>Дата створення: ${new Date(
                              notification.created_at
                            ).toLocaleDateString("uk-UA")}.</p>
                            <p>Техніка: ${
                              notification.order?.items
                                ?.map((item) => item.name)
                                .join(", ") || "Невідома техніка"
                            } (Кількість: ${
                      notification.order?.items?.length || 1
                    }).</p>
                            <p>Період оренди: ${
                      ...
                      notification.order?.rentalEnd
                        ...
                        : "невідомо"
                            ...
                            <p>Дата доставки: ${
                              notification.order?.deliveryDate
                                ? new Date(
                                    notification.order.deliveryDate
                                  ).toLocaleDateString("uk-UA")
                                : "невідомо"
                            ...
                            <p>Телефон користувача: ${
                              notification.user.phone
                            }.</p>
                        </div>
                        <div class="notification-actions">
                            <button class="action-button confirm-button" onclick="confirmNotification(${
                              notification.id
                            })">Створити договір</button>
                            <button class="action-button decline-button" onclick="deleteNotification(${
                              notification.id
                            })">Видалити</button>
                        </div>
                    </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.showModal = (modalId) => {
    ...
    if (modal) {
      ...
      showNotification(
        `Помилка: модальне вікно з id="${modalId}" не знайдено. Перевірте HTML.`
      );
    }
  };
  ...
  window.showEditModal = () => {
    fetch("https://agrofleet-pdqw.onrender.com/api/profile", {
      method: "GET",
      credentials: "include",
    })
      .then((response) => {
        console.log(
          "Відповідь від /api/profile (showEditModal):",
          response.status,
          response.statusText
        );
        if (!response.ok) {
      ...
      .then((user) => {
        ...
        document.getElementById("avatarFileName").textContent =
          user.profileImage ? "Поточний аватар завантажено" : "Файл не вибрано";
  ...
  window.login = async () => {
    ...
    try {
      const response = await fetch(
        "https://agrofleet-pdqw.onrender.com/api/auth/login",
        {
          method: "POST",
  ...
  window.register = async () => {
    ...
    try {
      const response = await fetch(
        "https://agrofleet-pdqw.onrender.com/api/auth/register",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      console.log(
        "Відповідь від /api/auth/register:",
        response.status,
        response.statusText
      );
      if (response.ok) {
  ...
  window.logout = () => {
    const logoutButton = document.querySelector(
      "#logoutModal .modal-button:not(.secondary)"
    );
    logoutButton.disabled = true;
    logoutButton.textContent = "Завантаження...";

    fetch("https://agrofleet-pdqw.onrender.com/api/auth/logout", {
      method: "POST",
      credentials: "include",
    })
      .then((response) => {
        console.log(
          "Відповідь від /api/auth/logout:",
          response.status,
          response.statusText
        );
        if (response.ok) {
    ...
    editForm.addEventListener("submit", async (e) => {
      ...
      if (avatarInput.files[0]) {
        ...
        if (avatarInput.files[0].size > maxSizeInBytes) {
          showNotification(
            `Файл аватара занадто великий! Максимальний розмір: ${maxSizeInMB} МБ.`
          );
          return;
        }
      }
...
async function updateProfile(user) {
  try {
    const response = await fetch(
      "https://agrofleet-pdqw.onrender.com/api/profile",
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    console.log(
      "Відповідь від /api/profile (updateProfile):",
      response.status,
      response.statusText
    );
    if (response.ok) {
    ...
    showNotification(
      "Помилка сервера при оновленні профілю. Перевірте підключення до мережі."
    );
  }
}
