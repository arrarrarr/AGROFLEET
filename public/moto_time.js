...
function initializeDatePickers() {
    ...
    flatpickr("#workDate", {
        ...
        locale: {
            ...
            months: {
                shorthand: ['Січ', 'Лют', 'Бер', 'Квіт', 'Трав', 'Черв', 'Лип', 'Серп', 'Вер', 'Жовт', 'Лист', 'Груд'],
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                console.log('Не авторизовано при отриманні даних про техніку. Завантажимо порожній список.');
                equipmentApiData = [];
                equipmentClasses = [];
                return;
            }
            throw new Error(errorData.error || `Failed to fetch equipment data: ${response.statusText}`);
        }

        equipmentApiData = await response.json();
        console.log('Дані про техніку завантажені з API:', equipmentApiData);

        equipmentClasses = [...new Set(equipmentApiData.map(equip => equip.type))];
        console.log('Класи техніки:', equipmentClasses);
    } catch (error) {
        console.error('Помилка при завантаженні даних про техніку з API:', error.message);
        equipmentApiData = [];
        equipmentClasses = [];
    }
}
...
async function calculateAverageConsumptionRates() {
    try {
        const response = await fetch('http://localhost:3000/api/moto_time', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            const errorData = await response.json();
            if (response.status === 401) {
                console.log('Не авторизовано при отриманні записів для середніх значень. Використаємо стандартні значення.');
                equipmentApiData.forEach(equip => {
            ...
            throw new Error(errorData.error || `Failed to fetch moto time records: ${response.statusText}`);
        }
        ...
        records.forEach(record => {
            ...
            if (equipmentStats[equipment]) {
                ...
                equipmentStats[equipment].totalFuel += parseFloat(record.fuel_consumed || 0);
                equipmentStats[equipment].totalOil += parseFloat(record.oil_consumed || 0);
                equipmentStats[equipment].totalLand += parseFloat(record.land_processed || 0);
...
function loadEquipmentOptions() {
    ...
    equipmentSelect.innerHTML = '<option value="" disabled selected>Оберіть техніку</option>';
...
function loadOperatorOptions() {
    ...
    operatorSelect.innerHTML = '<option value="" disabled selected>Оберіть оператора</option>';
...
function validateInput(input) {
    ...
    if (!input.value || (input.type === 'number' && input.value <= 0)) {
        ...
        errorMessage.textContent = input.id === 'hoursWorked' ? 'Введіть коректну кількість годин' : 'Це поле обов’язкове';
    } else {
...
async function loadData() {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            if (response.status === 401) {
                console.log('Не авторизовано при отриманні записів. Завантажимо тестові дані.');
                addTestData();
                return;
            }
            throw new Error(errorData.error || `Failed to fetch moto time records: ${response.statusText}`);
        ...
        records.forEach(record => {
            ...
            const equipmentClass = equipmentApiData.find(equip => equip.name === equipment)?.type || 'невідомий';
            ...
            totalHours[equipmentClass] = (totalHours[equipmentClass] || 0) + hoursWorked;
            totalFuel[equipmentClass] = (totalFuel[equipmentClass] || 0) + fuelConsumed;
            totalOil[equipmentClass] = (totalOil[equipmentClass] || 0) + oilConsumed;
            totalLand[equipmentClass] = (totalLand[equipmentClass] || 0) + landProcessed;
        });
...
function loadCharts() {
    const hoursTrendCtx = document.getElementById('hoursTrendCanvas')?.getContext('2d');
    if (!hoursTrendCtx) {
    ...
    const equipmentClassFilter = document.getElementById('equipmentClassFilter')?.value;

    // Фільтруємо записи з урахуванням вибраних фільтрів
    const filteredRecords = dailyRecords.filter(record => {
        const recordDate = new Date(record.date);
        const includeDate = recordDate >= dateRange.start && recordDate <= dateRange.end;
        const includeEquipment = !equipmentFilter || record.equipment === equipmentFilter;
        const equipmentClass = equipmentApiData.find(equip => equip.name === record.equipment)?.type;
        const includeClass = !equipmentClassFilter || equipmentClass === equipmentClassFilter;
    ...
    equipmentClasses.forEach(cls => {
        ...
        const hoursData = dates.map(date => {
            ...
            let totalHours = recordsOnDate.reduce((sum, record) => sum + (parseFloat(record.hours) || 0), 0);
        ...
        const landData = dates.map(date => {
            ...
            const totalLand = recordsOnDate.reduce((sum, record) => sum + (parseFloat(record.land_processed) || 0), 0);
            return totalLand;
        });
...
async function addWorkRecord() {
    try {
        ...
        if (!workDate || !workType || !equipment || !operator || isNaN(hoursWorked) || hoursWorked <= 0) {
            alert('Будь ласка, заповніть усі обов’язкові поля коректно!');
            return;
        }

        // Отримуємо дані про техніку
        const equipmentData = equipmentApiData.find(equip => equip.name === equipment);
        if (!equipmentData) {
        ...
        const modifier = workTypeConsumptionModifiers[workType] || { fuel: 1.0, oil: 1.0 };
        const fuelConsumed = (fuelConsumptionRate * hoursWorked * modifier.fuel).toFixed(2);
        const oilConsumed = (oilConsumptionRate * hoursWorked * modifier.oil).toFixed(2);
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || `Failed to add work record: ${response.statusText}`);
        }
        ...
        totalHours[newRecord.equipment] = (totalHours[newRecord.equipment] || 0) + parseFloat(newRecord.hours);
        totalFuel[newRecord.equipment] = (totalFuel[newRecord.equipment] || 0) + (newRecord.fuel_consumed || 0);
        totalOil[newRecord.equipment] = (totalOil[newRecord.equipment] || 0) + (newRecord.oil_consumed || 0);
        totalLand[newRecord.equipment] = (totalLand[newRecord.equipment] || 0) + (newRecord.land_processed || 0);

        totalHours[equipmentClass] = (totalHours[equipmentClass] || 0) + parseFloat(newRecord.hours);
        totalFuel[equipmentClass] = (totalFuel[equipmentClass] || 0) + (newRecord.fuel_consumed || 0);
        totalOil[equipmentClass] = (totalOil[equipmentClass] || 0) + (newRecord.oil_consumed || 0);
        totalLand[equipmentClass] = (totalLand[equipmentClass] || 0) + (newRecord.land_processed || 0);
...
async function deleteRecord(recordId) {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/moto_time/${recordId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || `Failed to delete work record: ${response.statusText}`);
        }
        ...
        if (record) {
            ...
            dailyRecords = dailyRecords.filter(record => record.id.toString() !== recordId);
...
function updateLogDisplay() {
    ...
    const filteredLogs = logs.filter(log => {
        ...
        const includeEquipment = !equipmentFilter || log.equipment === equipmentFilter;
        const includeClass = !equipmentClassFilter || log.equipmentClass === equipmentClassFilter;
        return includeDate && includeEquipment && includeClass;
    });
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="action-buttons">
                    <button class="action-btn delete-btn" data-id="${log.recordId}"><i class="fas fa-trash"></i></button>
                    <button class="action-btn export-btn" data-id="${log.recordId}"><i class="fas fa-file-pdf"></i></button>
                </div>
            </td>
        `;
    ...
    if (filteredLogs.length === 0) {
        logBody.innerHTML = '<tr><td colspan="10" class="no-data">Немає записів за обраний період.</td></tr>';
    }
...
function updateStatistics() {
    ...
    document.getElementById('totalHoursCount').textContent = `${totalHoursCount.toFixed(2)} год`;
    document.getElementById('totalFuelConsumption').textContent = `${totalFuelConsumption.toFixed(2)} л`;
    document.getElementById('totalOilConsumption').textContent = `${totalOilConsumption.toFixed(2)} л`;
    document.getElementById('totalLandProcessed').textContent = `${totalLandProcessed.toFixed(2)} га`;
    document.getElementById('totalOperatorsCount').textContent = operatorsSet.size;
}
