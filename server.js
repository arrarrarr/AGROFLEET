...
app.use((req, res, next) => {
  console.log(
    `[${new Date().toISOString()}] Перевірка сесії для запиту: ${req.method} ${
      req.url
    }`
  );
  console.log(
    "Ідентифікатор сесії:",
    req.sessionID || "Немає ідентифікатора сесії"
  );
  console.log("Дані сесії:", req.session || "Сесія не ініціалізована");
  console.log("Куки:", req.headers.cookie || "Немає куків");
  if (req.session && req.session.userId) {
    console.log(
      "Користувач авторизований. Ідентифікатор користувача:",
      req.session.userId
    );
  } else {
...
app.use((req, res, next) => {
  console.log(
    `[${new Date().toISOString()}] Новий запит: ${req.method} ${req.url}`
  );
  console.log("Куки у запиті:", req.headers.cookie || "Немає куків");
  console.log(
    "Ідентифікатор сесії:",
    req.sessionID || "Сесія не ініціалізована"
  );
  console.log("Дані сесії:", req.session || "Сесія не створена");
  if (req.session && req.session.userId) {
    console.log(
      "Сесія активна. Ідентифікатор користувача:",
      req.session.userId,
      "Закінчується:",
    ...
    if (req.sessionID && !req.session) {
      console.warn(
        "Сесія не ініціалізована, але ідентифікатор сесії присутній. Можлива проблема з SQLiteStore."
      );
    }
  }
  ...
  res.send = function (body) {
    console.log(
      `[${new Date().toISOString()}] Відповідь: ${req.method} ${
        req.url
...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "default-src 'self'; " +
      ...
      "connect-src 'self' https://agrofleet-pdqw.onrender.com/ wss://https://agrofleet-pdqw.onrender.com/;"
  );
  next();
});
...
app.use((req, res, next) => {
  if (!db) {
    ...
    return res
      .status(503)
      .json({ error: "Сервер ще ініціалізується, будь ласка, зачекайте..." });
  }
  req.db = db;
  console.log(
    "Middleware: Базу даних передано в запит. Ідентифікатор сесії:",
    req.sessionID || "Сесія не ініціалізована",
...
app.use("/api", (req, res, next) => {
  console.log(
    `[${new Date().toISOString()}] Перевірка авторизації для API-запиту: ${
      req.url
    }`
  );
...
app.get("/api/websocket-status", (req, res) => {
  try {
    ...
    res.json({
      status: "ok",
      message: "WebSocket-сервер працює",
...
app.get("/api/optimization/export", async (req, res) => {
  try {
    ...
    const tasks = await req.db.all(
      "SELECT * FROM optimization WHERE user_id = ?",
      [req.session.userId]
    );
...
const authenticatePage = (req, res, next) => {
  ...
  if (redirectCount >= 2) {
    ...
    return res
      .status(400)
      .send(
        "Помилка: виявлено цикл перенаправлень. Будь ласка, увійдіть у систему."
      );
  }
  ...
  console.log(
    "Авторизований доступ до сторінки:",
    req.url,
    "Ідентифікатор користувача:",
    req.session.userId
  );
...
const checkAdminOrManagerRoleForPage = async (req, res, next) => {
  try {
    ...
    const user = await req.db.get("SELECT role FROM users WHERE id = ?", [
      req.session.userId,
    ]);
    if (!user) {
    ...
    if (user.role !== "admin" && user.role !== "manager") {
      console.log(
        "Доступ до сторінки заборонено для користувача з роллю:",
        user.role,
        "на сторінку:",
        req.url
      );
    ...
    console.log(
      "Доступ до сторінки дозволено для користувача з роллю:",
      user.role,
      "на сторінку:",
      req.url
    );
...
app.get("/profile.html", (req, res) => {
  console.log(
    "Доступ до profile.html. Ідентифікатор сесії:",
    req.sessionID,
    "Ідентифікатор користувача:",
    req.session?.userId
  );
...
adminManagerPages.forEach((file) => {
  app.get(
    `/${file}`,
    authenticatePage,
    checkAdminOrManagerRoleForPage,
    (req, res) => {
...
app.get("/api/equipment-classes", authenticate, async (req, res) => {
  try {
    console.log(
      "GET /api/equipment-classes: Запит на список категорій техніки"
    );
    res.json(VALID_EQUIPMENT_CLASSES);
  } catch (err) {
    console.error(
      "Помилка при отриманні списку категорій техніки:",
      err.message
    );
    res
      .status(500)
      .json({ error: "Помилка сервера при отриманні списку категорій." });
  }
});
...
app.get("/api/expense-types", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при отриманні списку типів витрат." });
  }
});
...
app.get("/api/efficiency", authenticate, async (req, res) => {
  try {
    ...
    if (user.role !== "admin" && user.role !== "manager") {
      console.log(
        "Доступ до /api/efficiency заборонено для користувача з роллю:",
        user.role
      );
      return res
        .status(403)
    ...
    const efficiencyData = {
      users: users.length,
      motoHours: motoTimeRecords.reduce(
        (sum, record) => sum + (record.hours || 0),
        0
      ),
      rentals: rentals.length,
      usageHours: usageRecords.reduce(
        (sum, record) => sum + (record.hours || 0),
        0
      ),
      totalExpenses: expenseRecords.reduce(
        (sum, expense) => sum + (expense.expense_amount || 0),
        0
      ),
      totalContracts: contracts.length,
      averageMotoHoursPerRental: rentals.length
        ? (
            motoTimeRecords.reduce(
              (sum, record) => sum + (record.hours || 0),
              0
            ) / rentals.length
          ).toFixed(2)
        : 0,
      contractsPerUser: users.length
        ? (contracts.length / users.length).toFixed(2)
        : 0,
    };
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при отриманні даних про ефективність." });
  }
});
...
app.use((err, req, res, next) => {
  console.error(
    `[${new Date().toISOString()}] Необроблена помилка на маршруті ${
      req.method
    } ${req.url}:`,
    err.stack
  );
...
process.on("SIGINT", async () => {
  try {
    ...
    console.error(
      "Помилка при закритті бази даних або WebSocket-сервера:",
      err.message
    );
    process.exit(1);
  }
