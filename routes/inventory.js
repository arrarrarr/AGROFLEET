...
const validateInventory = (item) => {
  ...
  if (
    ...
    item.responsible.trim().length < 2
  ) {
    errors.push(
      "Відповідальна особа повинна бути рядком довжиною не менше 2 символів"
    );
  }
...
const createExpenseFromInventory = async (db, item, userId) => {
  ...
  console.log(
    `Створено запис витрат: ${item.name}, Кількість: ${item.quantity}, Загальна вартість: ${totalCost} грн`
  );
};
...
router.get("/", checkPermissions, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при отриманні списку запасів" });
  }
});
...
router.post("/", checkPermissions, async (req, res) => {
  try {
    ...
    if (existingItem) {
      ...
      newItem = await req.db.get("SELECT * FROM inventory WHERE id = ?", [
        existingItem.id,
      ]);
    } else {
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при створенні/оновленні запаса" });
  }
});
...
router.put("/:id", checkPermissions, async (req, res) => {
  try {
    ...
    if (
      ...
      existingItem.user_id !== req.currentUserId
    ) {
      return res
        .status(403)
        .json({ error: "Недостатньо прав для редагування цього запаса" });
    }
    ...
    if (duplicateItem) {
      ...
      updatedItem = await req.db.get("SELECT * FROM inventory WHERE id = ?", [
        duplicateItem.id,
      ]);
    } else {
      await req.db.run(
...
router.delete("/:id", checkPermissions, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Недостатньо прав для видалення цього запаса" });
    }

    await req.db.run("DELETE FROM inventory WHERE id = ?", [id]);
    // Не удаляем записи в expenses, так как они нужны для истории
    await req.db.run(
      "DELETE FROM notifications WHERE type = ? AND relatedId = ?",
      ["low_stock", id]
    );
...
router.get("/expenses", checkPermissions, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при отриманні списку витрат" });
  }
});
...
router.post("/expenses", checkPermissions, async (req, res) => {
  try {
    if (req.userRole !== "admin" && req.userRole !== "manager") {
      return res
        .status(403)
        .json({ error: "Недостатньо прав для створення запису витрат" });
    }
    ...
    const newExpense = await req.db.get("SELECT * FROM expenses WHERE id = ?", [
      result.lastID,
    ]);
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при створенні запису витрат" });
  }
});
...
router.delete("/expenses/:id", checkPermissions, async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const existingExpense = await req.db.get(
    ...
    if (
      req.userRole !== "admin" &&
      req.userRole === "manager" &&
      existingExpense.user_id !== req.currentUserId
    ) {
      return res
        .status(403)
        .json({ error: "Недостатньо прав для видалення цього запису" });
    }
    ...
    res
      .status(500)
      .json({ error: "Помилка сервера при видаленні запису витрат" });
  }
});
